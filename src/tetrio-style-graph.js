...
const graphAxes = [
  { key: 'attack', label: 'ATTACK', dx: 0, dy: -1, labelX: centerX, labelY: centerY - outerRadius - 16, anchor: 'middle' },
  { key: 'speed', label: 'SPEED', dx: 1, dy: 0, labelX: centerX + outerRadius + 16, labelY: centerY + 4, anchor: 'start' },
  { key: 'defense', label: 'DEFENSE', dx: 0, dy: 1, labelX: centerX, labelY: centerY + outerRadius + 28, anchor: 'middle' },
  { key: 'cheese', label: 'CHEESE', dx: -1, dy: 0, labelX: centerX - outerRadius - 16, labelY: centerY + 4, anchor: 'end' },
];
...
function calculateTetrioStyleGraphValues(stats = {}) {
  return {
    attack: clampGraphValue(divideFinite(stats.apm, 120)),
    speed: clampGraphValue(divideFinite(stats.pps, 3)),
    defense: clampGraphValue(multiplyFinite(stats.dsSecond, 1.5)),
    cheese: clampGraphValue(divideFinite(stats.cheeseIndex, 80)),
  };
}

async function createTetrioStyleGraph(input = {}) {
  const svg = renderTetrioStyleGraphSvg(input);
  return renderTetrioSvgToPng(svg);
}

function renderTetrioStyleGraphSvg(input) {
...
function normalizeGraphSeries(input) {
  ...
  return entries.map((entry) => ({
    ...
    values: calculateTetrioStyleGraphValues(entry?.stats),
  }));
}
...
function renderDataSeries(entry, index, seriesCount) {
  ...
  const dataPoints = graphAxes.map((axis) => getPoint(axis, entry.values[axis.key]));
...
function clampGraphValue(value) {
  ...
  return Math.max(0, Math.min(maxGraphValue, number));
}
...
function divideFinite(value, divisor) {
  ...
  if (!Number.isFinite(normalizedValue) || !Number.isFinite(normalizedDivisor) || normalizedDivisor === 0) {
    return null;
  }

  return normalizedValue / normalizedDivisor;
}
...
export {
  ...
  graphAxes as tetrioStyleGraphAxes,
  graphHeight as tetrioStyleGraphHeight,
  graphOutputHeight as tetrioStyleGraphOutputHeight,
  graphOutputScale as tetrioStyleGraphOutputScale,
  graphOutputWidth as tetrioStyleGraphOutputWidth,
  graphWidth as tetrioStyleGraphWidth,
