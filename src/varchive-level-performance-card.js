...
function renderEntryTile({ entry, jacketDataUrl, showVArchiveLevel, x, y, width, height }) {
  ...
  const levelLabel = showVArchiveLevel
    ? `V-ARCHIVE ${entry?.floorName ?? '-'}`
    : `${entry?.difficulty ?? '-'} ${entry?.level ?? '-'}`;
