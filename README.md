...
- 지정 시간 뒤 멘션 알람
- `%질문` 형태의 Gemini/Gemma 채팅, 답장 맥락/멘션 맥락/이미지 첨부 처리, 채널별 대화 기억과 출처가 기록되는 영구 기억
...
| `/가르치기 정보:<내용>` | `%<내용> 기억해줘`, `%<내용> 기억해둬`, `%<내용> 기억해` | 서버별 영구 기억에 정보와 작성자를 저장합니다. 관련 답변에서 사용되면 작성자 멘션을 출처로 표시합니다. |
...
`/가르치기` 또는 `%` 문장에 `기억해줘`, `기억해둬`, `기억해`를 넣으면 정보가 서버별 영구 저장소에 저장됩니다. 나중에 관련 질문에 답하면서 그 정보를 사용하면 답변 아래에 `<@작성자>가 알려준 정보다냥.` 형식으로 출처가 붙습니다.
