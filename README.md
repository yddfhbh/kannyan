...
| - | 체스판 이미지 + `%백선`, `%흑선`, `%분석해봐`, `%답이 뭐야` 등 | 백/흑 차례만 적거나 `분석`, `풀기`, `답`, `다음 수`처럼 풀이를 원하는 자연스러운 표현을 보내면 FEN을 읽고 Stockfish 최선 수를 보여줍니다. 테두리나 화면 여백이 있어도 보드 영역을 자동으로 찾으며, 차례가 빠졌으면 백/흑을 물어봅니다. |
