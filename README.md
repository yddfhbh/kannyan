...
| - | `%lb [스탯] [인원] [페이지/랭크/국가]`, `%rlb [스탯] [인원] [페이지/랭크/국가]` | TETR.IO 리더보드를 보여줍니다. `%lb`는 높은 순, `%rlb`는 낮은 순이며 APM, PPS, VS, APL, LPM, Glicko, TR과 여러 파생 통계를 지원합니다. APL/LPM은 `%ts`와 같은 공식 계산식을 사용합니다. |
...
TETR.IO 계정 연동 기반 명령은 TETR.IO 프로필에 Discord 계정을 연결한 사용자에게만 동작합니다. `%teto`, `%tetra`, `%qp`, `%exqp`, `%ts`, `%lb`, `%rlb`, `%psq`, `%sq`, `%vs`는 멘션이나 답장 대상의 연결 계정도 사용할 수 있습니다.
...
`%검색 OpenAI Responses API`, `/검색 질문:오늘 서울 날씨`처럼 명시적으로 검색할 수도 있고, `%최신 엔비디아 발표 알려줘`, `%오늘 서울 날씨 어때`처럼 최신 정보가 필요한 질문은 자동으로 웹 검색 결과를 참고합니다. 가사·작품명처럼 일반 키워드로 분류하기 어려운 정보 요청도 검색하며, 첫 답변에서 확인하지 못했다고 판단되면 웹 검색을 강제해 한 번 더 답변을 보강합니다.
